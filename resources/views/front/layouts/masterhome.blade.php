...
<head>
    ...
    @php
        ...
        $isLocalHost = in_array($host, ['localhost', '127.0.0.1'], true) || str_starts_with($host, '192.168.');
        $shouldLoadTracking = app()->environment('production') && !$isLocalHost;
    @endphp

    @if ($shouldLoadTracking)
        <script async src="https://www.googletagmanager.com/gtag/js?id=G-139HDX1VWS"></script>
        <script>
            window.dataLayer = window.dataLayer || [];
            function gtag() { dataLayer.push(arguments); }
            gtag('js', new Date());
            gtag('config', 'G-139HDX1VWS');
        </script>

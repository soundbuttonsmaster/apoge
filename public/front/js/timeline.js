...
(function() {
  ...
  var HorizontalTimeline = function(element) {
		...
		this.eventsMinDistance =140; // min distance between two consecutive events (in px)
  ...
  function startAutoSlide(timeline) {
    timeline.autoSlideInterval = setInterval(function() {
      timeline.newDateIndex = (timeline.newDateIndex + 1) % timeline.date.length;
      selectNewDate(timeline, timeline.date[timeline.newDateIndex]);
    }, 3000); // 3000 milliseconds = 3 seconds
  }

  // Reset the auto-slide interval when the user manually selects a date
  function resetAutoSlide(timeline) {
    clearInterval(timeline.autoSlideInterval);
    startAutoSlide(timeline);
  }
  ...
  function translateTimeline(timeline, direction) { // translate timeline (and date elements)
	...
	function updateVisibleContent(timeline) { // show content of new selected date
	 ...
	 var timelines = document.getElementsByClassName('js-h--timeline');
  if (timelines.length > 0) {
    for (var i = 0; i < timelines.length; i++) {
      new HorizontalTimeline(timelines[i]);
    }
  }

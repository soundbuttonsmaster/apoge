...
class AdminSitemapController extends Controller
{
    public function index()
    {
        ...
        $lastGeneratedAt = File::exists($sitemapPath) ? date('Y-m-d H:i:s', filemtime($sitemapPath)) : null;

        return view('admin.sitemap.index', compact('lastGeneratedAt'));
    }
    ...
    private function buildSitemapXml(): string
    {
        ...
        foreach ($categories as $category) {
            ...
            foreach ($subcategories as $subcategory) {
                $subLastmod = $subcategory->updated_at ? $subcategory->updated_at->toAtomString() : $currentDate;
                $xml .= $this->addUrl($baseUrl . '/p/' . $category->slug . '/' . $subcategory->slug, $subLastmod, '0.85', 'weekly');
            }
        }
    ...
    private function addUrl(string $url, string $lastmod, string $priority, string $changefreq): string
    {
        return "  <url>\n" .
    ...
    private function addUrlWithImage(
        string $url,
        string $lastmod,
        string $priority,
        string $changefreq,
